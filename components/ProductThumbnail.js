...
export default function ProductThumbnail({ product }) {
  ...
  useEffect(() => {
    const check = CartProducts.findIndex(
      (x) => x.title.trim() === product.title.trim(),
    );
      ...
      <Link href={`/Product/${product.id}`}>
        <span className="h-full w-full cursor-pointer">
          ...
          <div className="space-y-2  py-7 pl-2 pr-4">
            <div
              className={`${darkMode ? "text-slate-800" : "text-orange-500"}  font-semibold truncate text-lg`}
            >
              {product.category}
            </div>
            ...
            <div
              className={`${darkMode ? "text-slate-800" : "text-orange-500"}  font-semibold `}
            >
              ${product.price}
            </div>
          </div>
